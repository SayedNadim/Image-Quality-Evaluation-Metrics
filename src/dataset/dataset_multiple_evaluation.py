...
class DatasetMultipleEvaluation(data.Dataset):
    def __init__(self, config, data_path, fid=False, fid_ground_truth_or_image='none'):
        super(DatasetMultipleEvaluation, self).__init__()
        self.config = config
        self.fid = fid
        ...
        if config['dataset_format'] == 'image':
            if config['dataset_with_subfolders']:
                ...
                self.img_samples = find_samples_in_subfolders(data_path)
            else:
                ...
                self.img_samples = [os.path.join(data_path, x) for x in
                                    os.listdir(data_path) if allowed_image_extensions(x)]
        elif config['dataset_format'] == 'file_list':
            self.ground_truth_samples = default_flist_reader(config['ground_truth_image_path'])
            self.img_samples = default_flist_reader(data_path)
...
def build_dataloader_multiple_evaluation(config, data_path, fid, fid_ground_truth_or_image, batch_size,
                                         num_workers, shuffle=False):
    dataset = DatasetMultipleEvaluation(
        config=config,
        data_path=data_path,
        fid=fid,
