...
def psnr_calculation(x: torch.Tensor, y: torch.Tensor, data_range: Union[int, float] = 1.0,
                     reduction: str = 'mean', convert_to_greyscale: bool = False) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    score: torch.Tensor = 20 * torch.log10(torch.tensor(data_range)) - 10 * torch.log10(mse)

    return _reduce(score, reduction), _reduce(mse, reduction)
